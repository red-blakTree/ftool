...
pub fn cleanup() -> Result<(), FtoolError> {
    ...
    let to_remove: &[&str] = &[
        ...
        UDEV_PM_PATH,
        XORG_PATH,
        XORG_NVIDIA_DISCRETE_CONF,
        EXTRA_XORG_PATH,
        OLD_EXTRA_XORG_PATH,
        LIGHTDM_SCRIPT_PATH,
        LIGHTDM_CONFIG_PATH,
        PRIME_DISCRETE_PATH,
        // 兼容旧版 EnvyControl 的路径
    ...
    // 恢复 SDDM Xsetup 备份（如果存在）
    let backup_path = format!("{}.bak", SDDM_XSETUP_PATH);
    match fs::read_to_string(&backup_path) {
        Ok(content) => {
            info!("已找到 SDDM Xsetup 备份，正在恢复...");
            if let Err(e) = create_file(SDDM_XSETUP_PATH, &content, true) {
                warn!("恢复 SDDM Xsetup 备份失败; error={}", e);
                let _ = fs::remove_file(&backup_path);
            }
        }
        Err(_) => {
            // 无备份文件或无法读取；检查当前文件是否由 ftool 生成
            if let Ok(content) = fs::read_to_string(SDDM_XSETUP_PATH)
                && content.contains("# Automatically generated by ftool")
            {
                debug!("清理 ftool 生成的 SDDM Xsetup");
                if let Err(e) = create_file(SDDM_XSETUP_PATH, SDDM_XSETUP_CONTENT, true) {
                    warn!("恢复 SDDM Xsetup 默认内容失败; error={}", e);
                }
            }
        }
    }

    // 恢复 GDM Init（仅在文件由 ftool 生成时才清理）
    let gdm_backup_path = format!("{}.bak", GDM_INIT_PATH);
    match fs::read_to_string(&gdm_backup_path) {
        Ok(content) => {
            info!("已找到 GDM Init 备份，正在恢复...");
            if let Err(e) = create_file(GDM_INIT_PATH, &content, true) {
                warn!("恢复 GDM Init 备份失败; error={}", e);
                let _ = fs::remove_file(&gdm_backup_path);
            }
        }
        Err(_) => {
            // 无备份文件；检查当前文件是否由 ftool 生成
            if let Ok(content) = fs::read_to_string(GDM_INIT_PATH)
                && content.contains("# Automatically generated by ftool")
            {
                debug!("清理 ftool 生成的 GDM Init 文件");
                let _ = fs::remove_file(GDM_INIT_PATH);
            }
        }
    }
...
pub fn runtime_power_off() -> Result<(), FtoolError> {
    ...
    for func_id in &functions {
        ...
        if let Ok(driver_path) = driver_link.read_link() {
            ...
            fs::write(&unbind_path, func_id)
                .map_err(|e| FtoolError::Gpu(format!("解绑 {} 失败: {}", func_id, e)))?;
        }
    }
    ...
    for func_id in &functions {
        ...
        fs::write(&remove_path, "1")
            .map_err(|e| FtoolError::Gpu(format!("移除 {} 失败: {}", func_id, e)))?;
    }
...
/// 同步等待后设置 PCI 运行时电源管理
///
/// 参考 system76-power：NVIDIA 驱动初始化后约 5 秒内不应修改电源管理属性，
/// 否则可能导致系统锁死。同步阻塞可避免后台线程因进程退出而夭折。
fn apply_power_control(pci_id: &str, mode: super::GpuMode) -> Result<(), FtoolError> {
    info!("等待 NVIDIA 驱动初始化完成（约 5 秒）...");
